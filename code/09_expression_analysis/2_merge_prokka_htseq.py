...
def merge_expression_and_annotation(count_file, prokka_tsv, output_file, min_expression=10):
    expressed_genes = extract_expressed_genes(count_file, min_expression)
    if not expressed_genes:
        return False  # Skip if no gene passes threshold
    ...
    for gene_id, count in expressed_genes.items():
        ...
        result.append((gene_id, product, count))
    ...
    with open(output_file, 'w', newline='') as out:
        ...
        writer.writerow(['Gene_ID', 'Product', 'Count'])
        for row in result:
            writer.writerow(row)
...
min_expression = 10  # Expression threshold

for loc in locations:
    if loc == 'SRR4342129':
        bin_numbers = bin_numbers_SRR4342129
    elif loc == 'SRR4342133':
        bin_numbers = bin_numbers_SRR4342133
    for i in bin_numbers:
        ...
        output_file = f"{loc}_{bin_name}_expressed.tsv"

        if os.path.exists(count_file) and os.path.exists(prokka_tsv):
            success = merge_expression_and_annotation(count_file, prokka_tsv, output_file, min_expression)
            if success:
